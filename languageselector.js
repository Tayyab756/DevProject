...
(function () {
    ...
    console.log(`URL ID: "${scriptIdFromUrl}"`);
    console.log(`Required ID: "${requiredId}"`);
    console.log(`Data Type of URL ID: ${typeof scriptIdFromUrl}`);
    console.log(`Data Type of Required ID: ${typeof requiredId}`);

    // Trim whitespace from both IDs before comparison
    const trimmedScriptIdFromUrl = scriptIdFromUrl ? scriptIdFromUrl.trim() : null;
    const trimmedRequiredId = requiredId.trim();

    console.log(`Trimmed URL ID: "${trimmedScriptIdFromUrl}"`);
    console.log(`Trimmed Required ID: "${trimmedRequiredId}"`);

    // Check if the ID from the URL matches the required ID
    if (trimmedScriptIdFromUrl === trimmedRequiredId) {
        console.log("Script ID matches. Proceeding with translation functionality.");
        ...
        console.log("Script ID does not match. Skipping translation functionality.");
        // You can optionally add code here to perform some other action
        // if the ID doesn't match, or simply do nothing.

        // Still create the language selector UI, even if translation is skipped
        document.addEventListener("DOMContentLoaded", () => {
            createLanguageSelector();
        });
    }

...
(function () {
    // Get the ID from the script src URL parameter
    function getScriptId() {
        const scripts = document.getElementsByTagName('script');
        for (let script of scripts) {
            if (script.src.includes('selectLanguagessss.js')) {
    ...
    // Verify the ID exists and has the correct format
    if (!trimmedScriptIdFromUrl || trimmedScriptIdFromUrl.length !== 10) {
        console.error("Invalid or missing ID parameter in script URL");
    ...
    async function translatePage(targetLang) {
        // Crucial Check: Only proceed if the ID from the script URL is valid
        if (!trimmedScriptIdFromUrl) {
            console.log("Translation disabled - Invalid or missing ID");
            const selectorElement = document.getElementById("language-selector");
            if (selectorElement) {
                selectorElement.disabled = true;
                selectorElement.title = "Translation unavailable (invalid ID)";
            }
            return;
        }
    ...
    function initialize() {
        ...
        if (!trimmedScriptIdFromUrl) {
             console.error("Initialization aborted: Invalid or missing script ID.");
             return; // Stop initialization
         }

        // ID is valid, proceed with creating the UI
